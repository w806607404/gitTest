 ...
 * 功能说明：设备信息导出模板
 */
public class ZhengShangUserExportExecInfo {
		/***************************************************************************
		 * @param fileName
		 *            EXCEL文件名称
		 * @param listTitle
			...
			try {
				...
				if(null!=paramsList && paramsList.size()>0){
					for (BusinessOrder obj : paramsList) {
						...
						String userName = StringUtil.isValidateStr(String.valueOf(obj.getUser_name()));
						String user_phone = StringUtil.isValidateStr(String.valueOf(obj.getUser_mobile()));
						...
						if(!StringUtil.isNull(userName)){
						   sheet.addCell(new Label(0, i, userName, wcf_left));
						}else{
							sheet.addCell(new Label(0, i, "-", wcf_left));
						}
						
						if(!StringUtil.isNull(user_phone)){
							   sheet.addCell(new Label(1, i, user_phone, wcf_left));
						}else{
