 ...
 * 功能说明：在线报修记录导出模板
 */
public class OnlineRepairExportExecInfo {
		/***************************************************************************
		 * @param fileName
		 *            EXCEL文件名称
		 * @param listTitle
		...
		public final static String exportExcel(String fileName, String[] Title,
				List<NmgRepairRecord>  list,HttpServletResponse response) {
			...
			try {
				...
				if(null!=list && list.size()>0){
					for (NmgRepairRecord obj : list) {
					    ...
					    String run_status = String.valueOf(obj.getRun_status());
						
						sheet.addCell(new Label(0, i, deviceId, wcf_left));
						sheet.addCell(new Label(1, i, StringUtil.isValidateStr(contacts), wcf_left));
						sheet.addCell(new Label(2, i, StringUtil.isValidateStr(contact_phone), wcf_left));
						sheet.addCell(new Label(3, i, area_name, wcf_left));
						sheet.addCell(new Label(4, i, address, wcf_left));
						
					    if("0".equals(status)){
						    sheet.addCell(new Label(5, i, "未处理", wcf_left));
					    }else if("1".equals(status)){
					    	sheet.addCell(new Label(5, i, "处理中", wcf_left));
					    }else if("2".equals(status)){
					    	sheet.addCell(new Label(5, i, "已完成", wcf_left));
					    }else if("3".equals(status)){
					    	sheet.addCell(new Label(5, i, "已取消", wcf_left));
					    }
						
						sheet.addCell(new Label(6, i, create_time, wcf_left));
						
						if("1".equals(run_status)){
							sheet.addCell(new Label(7, i, "在线", wcf_left));
						}else if("0".equals(run_status)){
