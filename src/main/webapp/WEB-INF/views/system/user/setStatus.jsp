	...
	<form id="addForm" name="form" class="form-horizontal" method="post" action="${ctx}/user/update.shtml">
		...
		<div class="panel-body">
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input type="text" placeholder="用户昵称不能为空（必填）" class="form-control"  name="user_nickname" id="user_nickname" value="${accountInfo.user_nickname}" maxlength="32" readonly="true" style="cursor:hand">
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input type="text" placeholder="联系人不能为空（必填）" class="form-control"  name="agent_name" id="agent_name" maxlength="32" value="${accountInfo.agent_name}" readonly="true"  style="cursor:hand">
				</div>
			</div>
			<div class="form-group">
					<label class="col-sm-3 control-label">联系电话</label>
				<div class="col-sm-8">
					<input type="text" placeholder="联系电话不能为空（必填）" class="form-control"  name="agent_phone" id="agent_phone" maxlength="16" value="${accountInfo.agent_phone}" readonly="true" style="cursor:hand">
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label">状态</label>
				<div class="col-sm-8">
					<nobr><input name="status" type="radio" value="1" <c:if test="${accountInfo.status eq '1'}">checked</c:if> />开启  &nbsp;&nbsp;   <input name="status" type="radio" value="0" <c:if test="${accountInfo.status eq '0'}">checked</c:if>/>关闭  </nobr>
				</div>
			</div>
		</div>
...
function doSubmit(){
     layer.confirm('请检查数据是否正确?', function(index) {
		...
		if(data.status == 200){
			...
			layer.msg("该区域已有开启或审核通过的账号存在，不允许存在多个");
			return false;
		}
		else{
			layer.msg("修改失败");
