			...
			<div class="panel-body">
				<form id="formSearchAccountBack" class="form-horizontal">
					...
					<div class="form-group">
						...
						<button type="button" id="btn_query_userback"
...
<script>
	...
	$(function() {
		...
		pageTable = $ScinanPageTable_({
			$id : $('#userBackList'),
			url : "${ctx}/userBack/fetchUserBackByPage.shtml",//请求url
		...
		//查询按钮
		$('#btn_query_userback').click(function() {
		...
		$('#')
				.click(
						function() {
							...
							if (data.length == 0) {
								layer
										.msg("<spring:message code='select_one'/>");
								return;
							}

							if (data.length > 1) {
								layer
										.msg("<spring:message code='select_only_one'/>");
								return;
							}
							...
							$ScinanSwal
									.confirm(
											...
											function() {
												var obj = CommnUtil
														.ajax(
																"${ctx}/purchase/updateStatus.shtml",
																{
																	id : data[0].id
																}, "json");
												if (obj.status == 200) {
													$ScinanSwal
															.alert(
																	"<spring:message code='success_operat'/>",
																	"success");
													pageTable.refreshPage();
												} else if (obj.status == -1) {
													$ScinanSwal.alert("删除失败",
															"该用户还存在对应的分成比例!",
															"error");
													pageTable.refreshPage();
												} else {
													layer
															.msg("<spring:message code='delete_fail'/>");
												}
											});
						});

		//修改
		$('#btn_1005002001').click(
				function() {
					var data = pageTable.getSelections();
					...
					pageii = layer.open({
						...
						fix : true, //不固定
						maxmin : true,
						content : '${ctx}/purchase/modifyShipping.shtml?id='
								+ data[0].id
					});
